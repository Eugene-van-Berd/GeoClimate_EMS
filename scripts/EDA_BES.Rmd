...
brain_data %>%
  ...
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>% 
  ggplot()+
  geom_boxplot(aes(Dst_var, factor(year(Date)),
                   fill = factor(year(Date))), show.legend = FALSE)+
...
brain_data %>%
  mutate(Dst_var = Dst_max - Dst_min) %>%
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>%
  pull(Dst_var) %>% quantile(0.75), 
 
brain_data %>%
  mutate(Dst_var = Dst_max - Dst_min) %>%
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>%
  filter(Dst_var > quantile(Dst_var, 0.75, na.rm = TRUE)) %>%
  mutate(Dst_group = ntile(Dst_var, 4)) %>%
  group_by(Dst_group) %>%
  summarise(max = max (Dst_var)) %>% pull(max) %>% head(3))

brain_data %>%
  mutate(Dst_var = Dst_max - Dst_min) %>% 
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>% 
  mutate(Dst_group = case_when(
      Dst_var <= Dst_breaks[1] ~ 1,
      Dst_var <= Dst_breaks[2] ~ 2,
      Dst_var <= Dst_breaks[3] ~ 3,
      Dst_var <= Dst_breaks[4] ~ 4,
      TRUE ~ 5 ),
      Dst_group = factor(Dst_group, levels = 1:5, ordered = TRUE) 
  ) %>% 
  ggplot()+
  geom_histogram(aes(Dst_var, fill = Dst_group), binwidth = 1)+
  geom_boxplot(aes(Dst_var, -5), outliers = FALSE, width = 10) +
  scale_fill_brewer(type = "seq", palette = 7)+
  coord_cartesian(ylim = c(-5, 50))+ labs(y = "")+
  theme_custom
...
- Индекс Dst_var можно перевести в факторную переменную **Dst_group**, где
1 - значения Dst_var <= Q3 за наблюдаемый период
2, 3, 4, 5 - группы с одинаковым значением наблюдений, отличающиеся ростом Dst_var

- При визуализации Dst_group просматриватся положительная связь между количеством солнечных пятен и порядком группы, 
а для данных по вызовам скорой помощи связи не наблюдается
...
cor_data2 <- brain_data %>% 
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>% 
  transmute(EMS, Wind, Temp_mean, H2O,
