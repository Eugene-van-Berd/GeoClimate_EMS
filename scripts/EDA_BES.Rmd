...
brain_data %>% 
  mutate(Dst_var = Dst_max - Dst_min) %>% 
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>%
  pull(EMS) %>% ts(start = 2009, frequency = 365) %>%
...
WDC_Kyoto_Dst <- read_fwf("../data/raw/WDC_Kyoto.dat",  
  ...
  transmute(
    ...
    across(X11:X34) %>%  set_names(paste0(seq(1, 24), "_h"))
    ) %>% 
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31")))
...
brain_data2 <- brain_data %>% 
  filter(Date %within% interval(ymd("2009-01-01"), ymd("2019-12-31"))) %>% 
  transmute(Date, Week = as.factor(week(Date)), Month = as.factor(month(Date)), 
            Quarter = as.factor(quarter(Date)), Year = as.factor(year(Date)), 
...
brain_data2 %>% 
  ggplot()+
  geom_histogram(aes(EMS), fill = "skyblue", color = "black", binwidth = 2)+
  theme_custom

brain_data2 %>%  
  group_by(Year, Month) %>% 
  summarise(EMS_avg = mean(EMS, na.rm = TRUE), .groups = "drop") %>% 
  ggplot(aes(x = Month, y = Year, fill = EMS_avg)) +
...
brain_data2 %>%  
  group_by(Year, Month) %>% 
  summarise(Sunspots_avg = mean(Sunspots, na.rm = TRUE), .groups = "drop") %>% 
  ggplot(aes(x = Month, y = Year, fill = Sunspots_avg)) +
  geom_tile(colour = "black") + 
  scale_fill_gradient(low = "green", high = "red") + 
  theme_custom
