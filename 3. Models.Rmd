...
residuals(poisson_m0, type = "response") %>% acf() %>% autoplot() + 
  ggtitle("Basic models") + theme_custom,  
residuals(poisson_m1, type = "response") %>% acf() %>% autoplot() + 
  ggtitle("Basic models + Day of the week") + theme_custom
)
...
```{r Age_Sex, fig.height=8, fig.width=16}

poisson_m3 <- glm(EMS ~ Date + Day_Week + Temp_mean + Sunspots + Kp_Sum + Age*Sex,
                               data = brain_data_long_transform, family = poisson)
...
# workdays vs weekend
poisson_m4 <- glm(EMS ~ Date + Day_Week + DayOff + 
...
# COVID-19
poisson_m5 <- glm(EMS ~ Date + Day_Week + DayOff + COVID +
                        Temp_mean + Sunspots + Kp_Sum + Age*Sex,
                               data = brain_data_long_transform, family = poisson)
...
tibble(Shift = shifts, RMSE = RMSE_values) %>%
  ...
  geom_line(aes(y = Sin_year, color = "Cycle Component"), linewidth = 2)+
  scale_color_manual(values = c("Temperature" = "deepskyblue3", "Cycle Component" = "tomato3")) +
...
## Final Model
```{r Final, fig.height=10, fig.width=20}
...
final_table %>%   
  filter(p.value < 0.05) %>% 
   transmute(
      Term = term, Estimate = formatC(estimate, format = "e", digits = 2), 
      `Lower CI` = formatC(conf.low, format = "e", digits = 2), 
      `Upper CI` = formatC(conf.high, format = "e", digits = 2), 
