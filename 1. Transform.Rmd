---
title: "Преобразование исходных данных"
author: "Евгений Бердинских, Марина Козырева, Антон Тихоненко, Олеся Цветкова"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, include=FALSE}

knitr::opts_chunk$set(
  echo = TRUE,
  fig.path = "data/pics/transform-"
  )

library(conflicted)
library(factoextra)
library(readxl)
library(skimr)
library(tidyverse)
library(zoo)

conflict_prefer("filter", "dplyr", "stats")
conflict_prefer("lag", "dplyr", "stats")

```

## Загрузка исходных файлов 

```{r read, message=FALSE, warning=FALSE}

# Данные по вызовам скорой помощи по поводу острого нарушения мозгового кровообращения в Иркутске (2006 - 2022 года)
brain_data_raw <- read_excel("data/raw/database_2024-11-09.xlsx")  

# Данные по вызовам скорой помощи по поводу сердечных заболеваний в Иркутске (2006 - 2022 года)
heart_data_raw <- read_excel("data/raw/database_2025-01-18.xlsx")  

# Индексы геомагнитной активности (Ap и Kp) из GFZ, Potsdam (2006 - 2022 года)
GFZ_Potsdam_Ap <- read_csv("data/raw/GFZ_Potsdam_Ap.csv")  
GFZ_Potsdam_Kp <- read_csv("data/raw/GFZ_Potsdam_Kp.csv") %>% select(Date, Kp_Sum)   

# Индексы геомагнитной активности (Dst) из WDC, Kyoto (2006 - 2022 года)
WDC_Kyoto_Dst <- read_fwf("data/raw/WDC_Kyoto.dat", fwf_widths(c(3, 2, 2, 1, 2, 2, 1, 1, 2, 4, rep(4, 24), 4))) %>%
  rowwise() %>%
  transmute(
    Date = as.Date(paste0(X9, X2, "-", X3, "-", X5)),
    Dst_mean = X35,  #Среднее значение Dst за сутки
    Dst_sum = sum(c_across(X11:X34), na.rm = TRUE), #Суммарное значение Dst за сутки
    Dst_max = max(c_across(X11:X34), na.rm = TRUE), #Максимальное значение Dst за сутки
    Dst_min = min(c_across(X11:X34), na.rm = TRUE), #Минимальное значение Dst за сутки
    across(X11:X34) %>%  set_names(paste0("H_", seq(1, 24))) #Индекс Dst на каждый час
  ) %>% 
  ungroup()

# Данные о событиях SSC (Внезапное начало бури) и SI (Внезапный импульс) из Observatori de l'Ebre (2006 - 2022 года)
Obs_Ebre_SSC <- tibble()

for (i in (2006:2022)) {
  
  link <- paste0("data/raw/SC_data/ssc_", i, "_d.txt")
  
  Obs_Ebre_SSC %>% rbind(
    read_tsv(link, col_names = "raw") %>%
      filter(str_starts(raw, paste0(i))) %>%
      transmute(
        Date = str_replace_all(str_trunc( raw, width = 10, ellipsis = ""), " ", "-") %>% as.Date(),
        Storm = factor(str_remove_all(raw, "-") %>% str_trunc( width = 2, side = "left", ellipsis = "" ),
          levels = c("SI", "SC"),
          labels = c("Sud_Imp", "Sud_Storm")
        ))) -> Obs_Ebre_SSC

  }

Obs_Ebre_SSC <- Obs_Ebre_SSC %>%
  count(Date, Storm) %>%
  pivot_wider(names_from = Storm, values_from = n)


# Погода в Иркутске из Росгидромет (1882 - 2023 года)
Roshydromet_Irkutsk <- read_fwf("data/raw/Roshydromet_Irkutsk.dat", fwf_widths(c(
  5, 1, 4, 1, 2, 1, 2, 1, 1, 1, 5, 1, 1, 1, 5, 1, 1, 1, 5, 1, 1, 1, 5, 1, 1, 1
))) %>%
  select(where(is.numeric)) %>%
  transmute(
    Date = as.Date(paste0(X3, "-", X5, "-", X7)),
    Temp_mean = X15,  # Температура за день (средняя, минимум, максиум)
    Temp_min = X11,
    Temp_max = X19,
    H2O = X23 # Количество осадков за день
  ) 

#Международное число солнечных пятен из SIDC, Brussels (1818 - 2024 года)
SIDC_SILSO <- read_delim("data/raw/SIDC_SILSO.csv", delim = ";", col_names = FALSE)  %>% 
  transmute(Date = as.Date(paste0(X1, "-", X2, "-", X3)), 
            Sunspots = as.numeric(X5))



# Данные о локальном K индексе из магнитной обсерватории Иркутск (п. Патроны) 
K_IRT <- tibble()

for (i in (2006:2009)){
  
  link <- paste0("data/raw/K_IRT/IRT", i, ".txt")
  
  K_IRT %>% rbind(
    read_table(link, show_col_types = FALSE, col_names = FALSE, skip = 2) %>% 
      rowwise() %>% 
      mutate(
        across(everything(), as.character),
        IRT = list(tail(na.omit(c_across(everything())), 11))
      ) %>% 
      ungroup() %>% 
      unnest_wider(IRT,names_sep = "_") %>% 
      select(starts_with("IRT")) %>% 
      filter(IRT_1 != "IRT") %>% 
      transmute(
        Date = seq.Date(from = as.Date(paste(i, "-01-01", sep = "")), by = "day", length.out = n()), 
        K_00 = IRT_3, K_03 = IRT_4, K_06 = IRT_5, K_09 = IRT_6,
        K_12 = IRT_7, K_15 = IRT_8, K_18 = IRT_9, K_21 = IRT_10, 
        K_Sum = IRT_11, across(starts_with("K"), as.numeric)
      )) -> K_IRT
}

K_IRT <- K_IRT %>% 
  mutate(across(starts_with("K"), ~ifelse(. == -1, NA, .)))

for (i in (2012:2022)){
  
  link <- paste0("data/raw/K_IRT/IRT", i, ".txt")

  K_IRT %>% rbind(
  read_table(link, show_col_types = FALSE, col_names = FALSE, skip = 1) %>% 
  distinct() %>% filter(!is.na(X5)) %>%
  transmute(
    Date = X1, 
    Point = paste0("K_", substr(X2, 1, 2)), 
    K_IRT = as.numeric(gsub(";", "", X4))
  ) %>% 
  pivot_wider(names_from = Point, values_from = K_IRT) %>% 
  rowwise() %>% 
  mutate(across(starts_with("K"), ~ifelse(. == -1, NA, .)),
         K_Sum = sum(c_across(starts_with("K")), na.rm = FALSE)
         )) -> K_IRT
}
  
K_IRT <- 
  K_IRT %>% select(Date, K_Sum) 

```

## Оценка исходных данных

```{r skim_raw}

skim(brain_data_raw)
skim(heart_data_raw)

```


## Преобразование Dst индекса

```{r Dst}

#Метод главных компонент 
Dst_pca <- WDC_Kyoto_Dst %>% select(starts_with("H_")) %>% scale() %>% prcomp()
summary(Dst_pca)

fviz_eig(Dst_pca, addlabels = T, 
         xlab = "", ylab = "Процент дисперсии",
         main = "Объясненная дисперсия компонентов PCA")

#Преобразование WDC_Kyoto_Dst
Dst_transform <- WDC_Kyoto_Dst %>% 
  transmute(Date, Dst_sum, Dst_mean, 
            Dst_var = Dst_max - Dst_min, #Размах значений Dst за сутки
            Dst_level = factor(case_when(#Dst через факторную переменную
              Dst_min < -150 ~ "Severe", 
              Dst_min < -100 ~ "Major", 
              Dst_min < -50 ~ "Moderate",  
              Dst_min < -20 ~ "Weak",
              TRUE ~ "Quiet"), 
              levels = c("Quiet", "Weak", "Moderate", "Major", "Severe"), ordered = TRUE)
            ) %>% bind_cols( # Dst через главные компоненты
              as_tibble(Dst_pca$x) %>% select(PC1, PC2) %>% rename(Dst_PC1 = PC1, Dst_PC2 = PC2) )

```


## Подготовка файлов для анализа

```{r tidy}

# Вызов скорой медицинской помощи по поводу острого нарушения мозгового кровообращения 
brain_data <- brain_data_raw %>% 
  transmute(
    Date = dmy(Дата), Month = as.factor(month(Date)), Year = as.factor(year(Date)), 
    EMS = `Вызов СМП с диагнозом ОНМК у всех пациентов старше 25 лет`, 
    #Пропуски в данных заменяем на среднее по соседним значениям:
    F10.7 = `Cкорректированный (F10.7)` %>% na.approx(na.rm = FALSE), 
    Wind = ifelse(is.na(`Скорость ветра`), na.approx(`Скорость ветра`, na.rm = FALSE), `Скорость ветра`), 
    Pressure = `Атмосферное давление в среднем за сутки, С (Т)` %>%  
      ifelse(. < 600, NA, .) %>% na.approx(na.rm = FALSE)
  ) %>% 
  filter(!is.na(Date)) %>%
  left_join(Roshydromet_Irkutsk, by = join_by(Date)) %>%  #Данные о температуре и осадках
  left_join(SIDC_SILSO, by = join_by(Date)) %>%   #Данные о солнечных пятнах
  left_join(K_IRT, by = join_by(Date)) %>%  #Данные о локальном K индексе в Иркутске
  left_join(GFZ_Potsdam_Kp, by = join_by(Date)) %>%  #Данные о индексе Kp
  left_join(GFZ_Potsdam_Ap, by = join_by(Date)) %>%  #Данные о индексе Ap 
  left_join(Dst_transform, by = join_by(Date)) %>%  #Данные о индексе Dst 
  left_join(Obs_Ebre_SSC, by = join_by(Date)) %>%  #Данные о магнитных бурях
  mutate( across( starts_with("Sud"), ~ factor( case_when(is.na(.) ~ "No",
                                                             TRUE ~ "Yes") ) ) ) %>% 
  mutate(COVID = as.factor(ifelse(Date >= "2020-03-27", "Yes", "No")))

# Вызов скорой медицинской помощи по поводу сердечно-сосудистых заболеваний
heart_data <- brain_data %>% 
  select(!EMS) %>% 
  left_join(
    heart_data_raw %>% 
      transmute( 
        Date = seq.Date(from = as.Date("2006-11-29"), by = "day", length.out = n()), 
        #Нестабильная стенокардия (Unstable angina)
        EMS_UA = `Вызова СМП с диагнозом НС у всех пациентов старше 25 лет`,   
        #Острый инфаркт миокарда (Acute Myocardial Infarction)
        EMS_AMI = `Вызова СМП с диагнозом ОИМ у всех пациентов старше 25 лет`
        ),
    by = join_by(Date)) 

```

## Подготовка длинных файлов для анализа

```{r long}

# Вызов скорой медицинской помощи по поводу острого нарушения мозгового кровообращения 
brain_data_long <- brain_data_raw %>% 
  transmute(
    Date = dmy(Дата), Month = as.factor(month(Date)), Year = as.factor(year(Date)), 
    
    #Данные скорой помощи с разбивкой по полу и возрасту
    EMS_Male_1 = `Количество вызовов СМП с диагнозом ОНМК у мужчин 25-44 лет`,
    EMS_Male_2 = `Количество вызовов СМП с диагнозом ОНМК у мужчин 45-54 лет`,
    EMS_Male_3 = `Количество вызовов СМП с диагнозом ОНМК у мужчин старше 55 лет`,
    EMS_Female_1 = `Количество вызовов СМП с диагнозом ОНМК у женщин 25-44 лет`,
    EMS_Female_2 = `Количество вызовов СМП с диагнозом ОНМК у женщин 45-54 лет`,
    EMS_Female_3 = `Количество вызовов СМП с диагнозом ОНМК у женщин старше 55 лет`,
    
    #Пропуски в данных заменяем на среднее по соседним значениям:
    F10.7 = `Cкорректированный (F10.7)` %>% na.approx(na.rm = FALSE), 
    Wind = ifelse(is.na(`Скорость ветра`), na.approx(`Скорость ветра`, na.rm = FALSE), `Скорость ветра`), 
    Pressure = `Атмосферное давление в среднем за сутки, С (Т)` %>%  
      ifelse(. < 600, NA, .) %>% na.approx(na.rm = FALSE)
  ) %>% 
  
  filter(!is.na(Date)) %>%
  left_join(Roshydromet_Irkutsk, by = join_by(Date)) %>%  #Данные о температуре и осадках
  left_join(SIDC_SILSO, by = join_by(Date)) %>%   #Данные о солнечных пятнах
  left_join(K_IRT, by = join_by(Date)) %>%  #Данные о локальном K индексе в Иркутске
  left_join(GFZ_Potsdam_Kp, by = join_by(Date)) %>%  #Данные о индексе Kp
  left_join(GFZ_Potsdam_Ap, by = join_by(Date)) %>%  #Данные о индексе Ap 
  left_join(Dst_transform, by = join_by(Date)) %>%  #Данные о индексе Dst 
  left_join(Obs_Ebre_SSC, by = join_by(Date)) %>%  #Данные о магнитных бурях
  mutate( across( starts_with("Sud"), ~ factor( case_when(is.na(.) ~ "No",
                                                             TRUE ~ "Yes") ) ) ) %>% 
  mutate(COVID = as.factor(ifelse(Date >= "2020-03-27", "Yes", "No"))) %>% 
  
  # Длинный формат данных, 1 день - 6 строк
  pivot_longer(cols = starts_with("EMS_"), 
               names_to = c("Sex", "Age"), 
               names_pattern = "EMS_(Male|Female)_(1|2|3)", 
               values_to = "EMS"
  ) %>% 
  mutate(Sex = as.factor(Sex), 
         Age = as.factor(case_when(
         Age == 1 ~ "25-44", Age == 2 ~ "45-54", Age == 3 ~ "55+"
                         ))) 

# Вызов скорой медицинской помощи по поводу нестабильной стенокардии (Unstable angina)
UA_data_long <- brain_data %>% 
  select(!EMS) %>% 
  left_join(
    heart_data_raw %>% 
      transmute( 
        Date = seq.Date(from = as.Date("2006-11-29"), by = "day", length.out = n()), 
        
        #Данные скорой помощи с разбивкой по полу и возрасту
        EMS_Male_1 = `Количество вызовов СМП с диагнозом НС у мужчин 25-44 лет`,
        EMS_Male_2 = `Количество вызовов СМП с диагнозом НС у мужчин 45-54 лет`,
        EMS_Male_3 = `Количество вызовов СМП с диагнозом НС у мужчин старше 55 лет`,
        EMS_Female_1 = `Количество вызовов СМП с диагнозом НС у женщин 25-44 лет`,
        EMS_Female_2 = `Количество вызовов СМП с диагнозом НС у женщин 45-54 лет`,
        EMS_Female_3 = `Количество вызовов СМП с диагнозом НС у женщин старше 55 лет`
        ),
    by = join_by(Date)) %>% 
  
  # Длинный формат данных, 1 день - 6 строк
  pivot_longer(cols = starts_with("EMS_"), 
               names_to = c("Sex", "Age"), 
               names_pattern = "EMS_(Male|Female)_(1|2|3)", 
               values_to = "EMS"
  ) %>% 
  mutate(Sex = as.factor(Sex), 
         Age = as.factor(case_when(
         Age == 1 ~ "25-44", Age == 2 ~ "45-54", Age == 3 ~ "55+"
                         ))) 


# Вызов скорой медицинской помощи по поводу острого инфаркта миокарда (Acute Myocardial Infarction)
AMI_data_long <- brain_data %>% 
  select(!EMS) %>% 
  left_join(
    heart_data_raw %>% 
      transmute( 
        Date = seq.Date(from = as.Date("2006-11-29"), by = "day", length.out = n()), 
        
        #Данные скорой помощи с разбивкой по полу и возрасту
        EMS_Male_1 = `Количество вызовов СМП с диагнозом ОИМ у мужчин 25-44 лет`,
        EMS_Male_2 = `Количество вызовов СМП с диагнозом ОИМ у мужчин 45-54 лет`,
        EMS_Male_3 = `Количество вызовов СМП с диагнозом ОИМ у мужчин старше 55 лет`,
        EMS_Female_1 = `Количество вызовов СМП с диагнозом ОИМ у женщин 25-44 лет`,
        EMS_Female_2 = `Количество вызовов СМП с диагнозом ОИМ у женщин 45-54 лет`,
        EMS_Female_3 = `Количество вызовов СМП с диагнозом ОИМ у женщин старше 55 лет`
        ),
    by = join_by(Date)) %>% 
  
  # Длинный формат данных, 1 день - 6 строк
  pivot_longer(cols = starts_with("EMS_"), 
               names_to = c("Sex", "Age"), 
               names_pattern = "EMS_(Male|Female)_(1|2|3)", 
               values_to = "EMS"
  ) %>% 
  mutate(Sex = as.factor(Sex), 
         Age = as.factor(case_when(
         Age == 1 ~ "25-44", Age == 2 ~ "45-54", Age == 3 ~ "55+"
                         ))) 

```


## Оценка преобразованных файлов

```{r skim_tidy}

skim(brain_data)
skim(heart_data)
skim(brain_data_long)
skim(UA_data_long)
skim(AMI_data_long)


```

## Запись/чтение преобразованных файлов

```{r write, message=FALSE}

#Локальное храненение данных 
write_csv(brain_data, "data/raw/brain_data.csv") 
write_csv(heart_data, "data/raw/heart_data.csv") 
write_csv(brain_data_long, "data/raw/brain_data_long.csv") 
write_csv(UA_data_long, "data/raw/UA_data_long.csv") 
write_csv(AMI_data_long, "data/raw/AMI_data_long.csv") 

# Вызов скорой медицинской помощи по поводу острого нарушения мозгового кровообращения 
brain_data <- read_csv("data/raw/brain_data.csv") %>%
   mutate(across(c(Month, Year, Dst_level, Sud_Imp, Sud_Storm, COVID), as.factor))

brain_data_long <- read_csv("data/raw/brain_data_long.csv") %>%
   mutate(across(c(Month, Year, Dst_level, Sud_Imp, Sud_Storm, Sex, Age, COVID), as.factor))

# Вызов скорой медицинской помощи по поводу сердечно-сосудистых заболеваний
heart_data <- read_csv("data/raw/heart_data.csv") %>%
   mutate(across(c(Month, Year, Dst_level, Sud_Imp, Sud_Storm, COVID), as.factor))

# Вызов скорой медицинской помощи по поводу нестабильной стенокардии (Unstable angina)
UA_data_long <- read_csv("data/raw/UA_data_long.csv") %>%
   mutate(across(c(Month, Year, Dst_level, Sud_Imp, Sud_Storm, Sex, Age, COVID), as.factor))

# Вызов скорой медицинской помощи по поводу острого инфаркта миокарда (Acute Myocardial Infarction)
AMI_data_long <- read_csv("data/raw/AMI_data_long.csv") %>%
   mutate(across(c(Month, Year, Dst_level, Sud_Imp, Sud_Storm, Sex, Age, COVID), as.factor))

```

